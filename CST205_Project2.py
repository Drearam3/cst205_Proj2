...
##  Description: This python program is a photomosaic that asks the user for an image, via GUI button, and then takes in that image
##                  and creates an empty image based on its proportions after it is scaled down to a width of at least 100 pixels
##                  (the height depends on the width). The program then sorts through given "tile" images that compose of many colors
##                  and chooses the tile image that best matches each individual pixel of the photo to be mosaicked. When the program
##                  is finished going through all of the pixels of the big picture, then it saves the new image to a file call "Original.jpg"
##                  and displays the image. 
##  Trello Link: https://trello.com/b/IzkfN0Vz
##  GitHub Link: https://github.com/Drearam3/cst205_Proj2.git
...
def create_mosaic(original_picture, file):
    ...
    path = os.path.dirname(file) + "/tiles/"
    ...
    heightFourth = org_height / 4;
    heightFourth = int(heightFourth)
    heightHalf = org_height / 2;
    heightHalf = int(heightHalf)
    height3Fourth = (org_height * 3) / 4;
    height3Fourth = int(height3Fourth)
            ...
            if(org_actual_name in my_actual):
                for i in range (0, tiles):
                    if (org_actual_name == my_actual[i]):
                        ...
                        i = tiles - 1
                        break      
        if(height == heightFourth):
            print("25% Done")
        if(height == heightHalf):
            print("50% Done")
        if(height == height3Fourth):
            print("75% Done")
        if(height == (org_height - 1)):
            print("Done!")
...
csumbPhoto = fd.PhotoImage(file = "GUI/csumbLogo.png")
mozaicWordPhoto = fd.PhotoImage(file = "GUI/mozaicGroupTxt.png")
